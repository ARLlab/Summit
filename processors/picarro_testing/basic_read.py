...
for name, standard in standards.items():
	...
	standard_events[:] = [ce for ce in standard_events if ce.duration.seconds >= 110]  # remove any cal events shorter than 110s
...
def plot_standard_compound(standard, compound, df):
	...
	if name == 'low_standard':
		ax.plot([df['date'].iloc[0], df['date'].iloc[-1]], [low_limit_dict.get(compound)[2], low_limit_dict.get(compound)[2]])
		ax.set_ylim(bottom=low_limit_dict.get(compound)[0], top=low_limit_dict.get(compound)[1])

	elif name == 'mid_standard':
		ax.plot([df['date'].iloc[0], df['date'].iloc[-1]], [mid_limit_dict.get(compound)[2], mid_limit_dict.get(compound)[2]])
		ax.set_ylim(bottom=mid_limit_dict.get(compound)[0], top=mid_limit_dict.get(compound)[1])

	elif name == 'high_standard':
		ax.plot([df['date'].iloc[0], df['date'].iloc[-1]], [high_limit_dict.get(compound)[2], high_limit_dict.get(compound)[2]])
